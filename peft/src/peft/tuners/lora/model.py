...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        # {'r': 8, 'lora_alpha': 8, 'lora_dropout': 0.0, 'fan_in_fan_out': False, 'init_lora_weights': True, 'use_rslora': False, 'use_dora': False, 'ephemeral_gpu_offload': False, 'loaded_in_8bit': False, 'loaded_in_4bit': False}
        
        # how lora_A * lora_B works 2024.11.29
        # 真的进来了...
        ...
        if isinstance(target, LoraLayer) and not isinstance(target, AdaLoraLayer):
            ...
            # 2024.11.29 enter this
            # target: Linear(in_features=3072, out_features=3072, bias=True)
            ...
            #   )
            #   (lora_A): ModuleDict(
            #     (default_0): Linear(in_features=3072, out_features=8, bias=False)
            #   )
            #   (lora_B): ModuleDict(
            #     (default_0): Linear(in_features=8, out_features=3072, bias=False)
            #   )
            ...
            # target_name: 'add_k_proj
            self._replace_module(parent, target_name, new_module, target)

    def _replace_module(self, parent, child_name, new_module, child):
        # 2024.11.29 直接替换原始权重为带lora的.
