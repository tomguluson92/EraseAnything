...
class FluxLoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_into_transformer(
        ...
        if len(state_dict.keys()) > 0:
            ...
            # low_cpu_mem_usage https://github.com/huggingface/peft/pull/2240
            low_cpu_mem_usage = False
            peft_kwargs = {}
            if is_peft_version(">=", "0.13.1"):
                peft_kwargs["low_cpu_mem_usage"] = low_cpu_mem_usage
            
            # 2024.11.29 how lora weights add to original weight?
            # Lora_A & Lora_B ?
            # LoraConfig(peft_type=<PeftType.LORA: 'LORA'>, auto_mapping=None, base_model_name_or_path=None, revision=None, task_type=None,  inference_mode=False, r=8, target_modules={'transformer_blocks.16.attn.add_k_proj', ... , 'transformer_blocks.3.attn.add_q_proj'}, lora_alpha=8, lora_dropout=0.0, fan_in_fan_out=False, bias='none', use_rslora=False, modules_to_save=None, init_lora_weights=True, layers_to_transform=None, layers_pattern=None, rank_pattern={}, alpha_pattern={}, megatron_config=None, megatron_core='megatron.core', loftq_config={}, use_dora=False, layer_replication=None, runtime_config=LoraRuntimeConfig(ephemeral_gpu_offload=False))
            ...
            # inject_adapter_in_model是为模型注入lora_A和lora_B的nn.Parameter
            # set_peft_model_state_dict才是seiz塞权重的地方
            # (Pdb) state_dict.keys()
            # dict_keys(['transformer_blocks.0.attn.add_k_proj.lora_A.weight', 'transformer_blocks.0.attn.add_k_proj.lora_B.weight', ...])
            inject_adapter_in_model(lora_config, transformer, adapter_name=adapter_name, **peft_kwargs)
            incompatible_keys = set_peft_model_state_dict(transformer, state_dict, adapter_name, **peft_kwargs)
