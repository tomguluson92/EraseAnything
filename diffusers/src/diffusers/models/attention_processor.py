...
# 2024.11.13 mask bandwith
def mask_bandwith(mask, bandwith=1):
    indices = torch.where(mask == 1)
    N = mask.shape[0]
    
    while bandwith >= 1:
        row_tmp = indices[0] + bandwith
        col_tmp = indices[1] + bandwith
        
        row_tmp_2 = indices[0] - bandwith
        col_tmp_2 = indices[1] - bandwith
        
        for idx in range(N):
            if int(row_tmp[idx]) >= N or int(col_tmp[idx]) >= N:
                continue
            else:
                mask[int(indices[0][idx]), int(col_tmp[idx])] = 1
                mask[int(row_tmp[idx]), int(indices[1][idx])] = 1
                
            
            if int(row_tmp_2[idx]) < 0 or int(col_tmp_2[idx]) < 0:
                continue
            else:
                mask[int(indices[0][idx]), int(col_tmp_2[idx])] = 1
                mask[int(row_tmp_2[idx]), int(indices[1][idx])] = 1
            
        bandwith -= 1
        
    return mask

aaa = torch.ones(1280, 1280)
# for i in range(1280):
    # aaa[i, 1280 - 1 - i] = 1
...
MASK_SAM[:256, :] = 0
MASK_SAM[:, 1024:] = 0
# MASK_SAM = mask_bandwith(aaa, 256)
# MASK_SAM[256:, :256] = 1
# MASK_SAM = torch.ones(1280, 1280)
# MASK_SAM[256:512, 768:] = 0
# MASK_SAM = 1 - MASK_SAM

# from PIL import Image
# import numpy as np
# mask_image = Image.fromarray(MASK_SAM.cpu().numpy().astype(np.uint8) * 255)
# mask_image.save('/openbayes/home/attention_map/mask_attn.png')
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # remove_index: Optional[List[int]] = None,
...
def pca_lowrank(A,n=5):
    U,S,V = torch.pca_lowrank(A, q=n, center=True, niter=2)
    return torch.matmul(U[:,:n], torch.diag(S[:n])).bfloat16()
...
def Fourier_filter(x, threshold, scale):
    ...
    crow, ccol = H // 2, W //2
    mask[..., crow - threshold:crow + threshold, ccol - (threshold//8):ccol + (threshold//8)] = scale
    x_freq = x_freq * mask
...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        # 2024.10.29 text image visualization.
        ...
        if encoder_hidden_states is not None:
            ...
            # import pdb; pdb.set_trace()
            # print("text embed", encoder_hidden_states_query_proj.shape)
            # print("image embed", query.shape)
            # print("="*30)
            # encoder_hidden_states_query_proj torch.Size([1, 24, 256, 128])
            # query                            torch.Size([1, 24, 1024, 128])
            
            # 2024.11.13 FreeU
#             query = Fourier_filter(query, threshold=100, scale=1.6)
#             key = Fourier_filter(key, threshold=100, scale=1.6)
            
#             hidden_mean_q = encoder_hidden_states_query_proj.mean(1).unsqueeze(1)
#             hidden_mean_k = encoder_hidden_states_key_proj.mean(1).unsqueeze(1)
#             B = hidden_mean_q.shape[0]
            
#             hidden_max_q, _ = torch.max(hidden_mean_q.view(B, -1), dim=-1, keepdim=True) 
#             hidden_min_q, _ = torch.min(hidden_mean_q.view(B, -1), dim=-1, keepdim=True)
#             hidden_mean_q = (hidden_mean_q - hidden_min_q.unsqueeze(2).unsqueeze(3)) / (hidden_max_q - hidden_min_q).unsqueeze(2).unsqueeze(3)
            
#             hidden_max_k, _ = torch.max(hidden_mean_k.view(B, -1), dim=-1, keepdim=True) 
#             hidden_min_k, _ = torch.min(hidden_mean_k.view(B, -1), dim=-1, keepdim=True)
#             hidden_mean_k = (hidden_mean_k - hidden_min_k.unsqueeze(2).unsqueeze(3)) / (hidden_max_k - hidden_min_k).unsqueeze(2).unsqueeze(3)
            
#             encoder_hidden_states_query_proj = encoder_hidden_states_query_proj * ((1.3 - 1) * hidden_mean_q + 1)
#             encoder_hidden_states_key_proj = encoder_hidden_states_key_proj * ((1.3 - 1) * hidden_mean_k + 1)
        ...
        # 2024.10.29
        # 1. self define scaled_dot_product_attention.
        # 2. visualize. TODO.
        vis_attn = True
        ...
        if not vis_attn:
            ...
            hidden_states, attn_maps = sam_scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False)
            # hidden_states, attn_maps = sam_scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False, attn_index=remove_index)
        ...
        if encoder_hidden_states is not None:
            ...
            if vis_attn:
                return hidden_states, encoder_hidden_states, attn_maps
            else:
                return hidden_states, encoder_hidden_states
        else:
            return hidden_states
